...
// start logging... , 'com_rsg2'
JLog::add('-------------------------------------------------------', JLog::DEBUG);
JLog::add('Starting to log install.rsg2.php for installation X', JLog::DEBUG);
...
// ToDO: #__schemas" Tabelle reparieren ??? -> http://vi-solutions.de/de/enjoy-joomla-blog/116-knowledgbase-tutorials
	...
	function preflight($type, $parent)
	{
        ...
        if ( $type == 'update' ) {
            ...
            //--- Determine rsg2 extension id ------------------
            $db = JFactory::getDbo();
            $query = $db->getQuery(true);
            $query->select($db->quoteName('extension_id'))
                ->from('#__extensions')
                ->where($db->quoteName('type') . ' = ' . $db->quote('component') 
					. ' AND ' . $db->quoteName('element') . ' = ' . $db->quote('com_rsg2') 
					. ' AND ' . $db->quoteName('name') . ' = ' . $db->quote('com_rsg2'));
            $db->setQuery($query);		
	...
	function install($parent)
	{
        ...
        // $parent->getParent()->setRedirectURL('index.php?option=com_rsg2');
